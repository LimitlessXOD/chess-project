...
const wK = (props) => (
  <svg viewBox="0 0 45 45" width="100%" height="100%" style={props?.svgStyle}>
    ...
    >
      ...
      <path d="M 22.5,25 C 22.5,25 27,17.5 25.5,14.5 C 25.5,14.5 24.5,12 22.5,12 C 20.5,12 19.5,14.5 19.5,14.5 C 18,17.5 22.5,25 22.5,25" style={{ fill: props?.fill ?? "#ffffff", stroke: "#000000", strokeLinecap: "butt", strokeLinejoin: "miter" }} />
      <path d="M 12.5,37 C 18,40.5 27,40.5 32.5,37 L 32.5,30 C 32.5,30 41.5,25.5 38.5,19.5 C 34.5,13 25,16 22.5,23.5 L 22.5,27 L 22.5,23.5 C 20,16 10.5,13 6.5,19.5 C 3.5,25.5 12.5,30 12.5,30 L 12.5,37" style={{ fill: props?.fill ?? "#ffffff", stroke: "#000000" }} />
...
const bQ = (props) => (
  <svg viewBox="0 0 45 45" width="100%" height="100%" style={props?.svgStyle}>
    ...
    >
      <path d="M 9,26 C 17.5,24.5 30,24.5 36,26 L 38.5,13.5 L 31,25 L 30.7,10.9 L 25.5,24.5 L 22.5,10 L 19.5,24.5 L 14.3,10.9 L 14,25 L 6.5,13.5 L 9,26 z" style={{ strokeLinecap: "butt", fill: props?.fill ?? "#000000" }} />
...
const bK = (props) => (
  <svg viewBox="0 0 45 45" width="100%" height="100%" style={props?.svgStyle}>
    ...
    >
      <path d="M 22.5,11.63 L 22.5,6" style={{ fill: "none", stroke: "#000000", strokeLinejoin: "miter" }} />
      <path d="M 22.5,25 C 22.5,25 27,17.5 25.5,14.5 C 25.5,14.5 24.5,12 22.5,12 C 20.5,12 19.5,14.5 19.5,14.5 C 18,17.5 22.5,25 22.5,25" style={{ fill: props?.fill ?? "#000000", fillOpacity: 1, strokeLinecap: "butt", strokeLinejoin: "miter" }} />
      <path d="M 12.5,37 C 18,40.5 27,40.5 32.5,37 L 32.5,30 C 32.5,30 41.5,25.5 38.5,19.5 C 34.5,13 25,16 22.5,23.5 L 22.5,27 L 22.5,23.5 C 20,16 10.5,13 6.5,19.5 C 3.5,25.5 12.5,30 12.5,30 L 12.5,37" style={{ fill: props?.fill ?? "#000000", stroke: "#000000" }} />
      <path d="M 20,8 L 25,8" style={{ fill: "none", stroke: "#000000", strokeLinejoin: "miter" }} />
...
// Helper lists for column/row mapping
const FILES = ["a", "b", "c", "d", "e", "f", "g", "h"];
const RANKS = ["8", "7", "6", "5", "4", "3", "2", "1"];

// Helper to parse FEN to 8x8 matrix
const parseFen = (fen) => {
  const board = Array(8).fill(null).map(() => Array(8).fill(null));
  if (!fen) return board;

  const fenString = fen === "start" ? "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1" : fen;
...
export function Chessboard({
  ...
  const boardData = useMemo(() => parseFen(position), [position]);

  const [draggedOverSquare, setDraggedOverSquare] = useState(null);

  // Setup visual grid mapping based on orientation
  const viewFiles = useMemo(() => {
    return boardOrientation === "black" ? [...FILES].reverse() : FILES;
  }, [boardOrientation]);

  const viewRanks = useMemo(() => {
    return boardOrientation === "black" ? [...RANKS].reverse() : RANKS;
  }, [boardOrientation]);

  // Handle Square click
  const handleSquareClick = (squareId, pieceCode) => {
    if (onSquareClick) {
      onSquareClick({ square: squareId, piece: pieceCode });
    }
  };

  // NATIVE HTML5 DRAG & DROP
  const handleDragStart = (e, squareId, pieceCode) => {
    if (!arePiecesDraggable) return;
    
    // Store data as JSON string for compatibility
    const dragData = JSON.stringify({ sourceSquare: squareId, piece: pieceCode });
    e.dataTransfer.setData("text/plain", dragData);
    e.dataTransfer.effectAllowed = "move";
    
    // Create an invisible drag image or style the element
  ...
  const handleDragOver = (e, squareId) => {
    e.preventDefault();
    if (draggedOverSquare !== squareId) {
      setDraggedOverSquare(squareId);
    }
  };
  ...
  const handleDrop = (e, targetSquareId) => {
    ...
    try {
      ...
      const { sourceSquare, piece } = JSON.parse(dragData);
      if (sourceSquare === targetSquareId) return;

      if (onPieceDrop) {
        onPieceDrop({ piece, sourceSquare, targetSquare: targetSquareId });
      }
    } catch (err) {
      console.error("Chessboard custom drop error:", err);
    }
  };
  ...
  for (let r = 0; r < 8; r++) {
    for (let c = 0; c < 8; c++) {
      const squareId = viewFiles[c] + viewRanks[r];

      // Map back standard FEN coordinates for piece lookup
      const stdCol = FILES.indexOf(viewFiles[c]);
      const stdRow = RANKS.indexOf(viewRanks[r]);
      const pieceCode = boardData[stdRow][stdCol];

      // Light or dark square style
      const isLight = (r + c) % 2 === 0;
      const defaultSquareStyle = isLight
        ? { backgroundColor: "#ECE3D4", ...customLightSquareStyle }
        : { backgroundColor: "#708A83", ...customDarkSquareStyle };

      // Highlight/Styles merging
      const highlightStyle = customSquareStyles[squareId] || {};
      
      // Drag overlay active style
      const isOver = draggedOverSquare === squareId;
      const dragOverStyle = isOver ? { filter: "brightness(1.15) contrast(1.1)", boxShadow: "inset 0 0 12px rgba(255,255,255,0.3)" } : {};
      ...
      const PieceComponent = pieceCode ? PIECES[pieceCode] : null;

      squares.push(
        <div
          key={squareId}
          className={`custom-board-square ${isLight ? "light" : "dark"} ${isOver ? "drag-over" : ""}`}
          style={combinedSquareStyle}
          onClick={() => handleSquareClick(squareId, pieceCode)}
        ...
        >
          {/* Notation labels */}
          {c === 0 && (
            <span className="notation-rank" style={{ color: isLight ? "#708A83" : "#ECE3D4" }}>
              {viewRanks[r]}
            </span>
          )}
          {r === 7 && (
            <span className="notation-file" style={{ color: isLight ? "#708A83" : "#ECE3D4" }}>
              {viewFiles[c]}
            </span>
          )}
          ...
          {PieceComponent && (
            <div
              className="custom-piece-container"
              draggable={arePiecesDraggable}
              onDragStart={(e) => handleDragStart(e, squareId, pieceCode)}
  ...
  return (
    <div className="custom-chessboard-wrapper" style={customBoardStyle}>
      <div className="custom-chessboard-grid">
        {squares}
      </div>
    </div>
  );
